...
# 2021-11-08 - vnahum/ mthouseau  : Migration on new s3 
...
=cut
my @list_to_externalize = (
  ['redcapcicdev', 's3://s3-aws-services-redcap-backups/redcap-cic'],
  ['redcapicm', 's3://s3-aws-services-redcap-backups/redcap-icm-staging']
)

foreach my $dump_todo (@list_to_externalize)
{
    print ("$number\n");
}
=cut
...
#my $bucket_backup   = "s3://s3-aws-services-redcap-backups";

my $dir_base        = "/tmp";
my $folder2         = "backup2";
my $dir_backup2      = "$dir_base/$folder2";
my $bucket_redcap_cic = "s3://s3-aws-services-redcap-backups/redcap-cic"
my $bucket_redcap_icm_staging = "s3://s3-aws-services-redcap-backups/redcap-icm-staging"
my $bucket_redcap_icm = "s3://s3-aws-services-redcap-backups/redcap-icm"
my $bucket_redcap_im2a = "s3://s3-aws-services-redcap-backups/redcap-im2a"
my $bucket_redcap_nspark = "s3://s3-aws-services-redcap-backups/redcap-nspark"
my $bucket_redcap_paraneo = "s3://s3-aws-services-redcap-backups/redcap-paraneo"
...
foreach my $db (@$databases){
        ...
        my $file2 = "$dir_backup2/$D2\_$db.sql";
...
#for my $i (7..10){
        ...
        my $cmd_backup = "mysqldump --user $user -p'$password' -R -B $db[$i] --single-transaction";
...
my $output_s3_file_exists = `/usr/bin/aws  --profile glpi-backup s3 ls $archive $bucket_backup/$archive 2>&1 1>/dev/null`;
